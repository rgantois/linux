...
extern int qca8k_mib_init(struct qca8k_priv *priv);
extern void qca8k_port_set_status(struct qca8k_priv *priv, int port, int enable);
extern int qca8k_read_switch_id(struct qca8k_priv *priv);

/* Common read/write/rmw function */
extern int qca8k_read(struct qca8k_priv *priv, u32 reg, u32 *val);
extern int qca8k_write(struct qca8k_priv *priv, u32 reg, u32 val);
extern int qca8k_rmw(struct qca8k_priv *priv, u32 reg, u32 mask, u32 write_val);

/* Common ops function */
extern void qca8k_fdb_flush(struct qca8k_priv *priv);

/* Common ethtool stats function */
extern void qca8k_get_strings(struct dsa_switch *ds, int port, u32 stringset, uint8_t *data);
extern void qca8k_get_ethtool_stats(struct dsa_switch *ds, int port,
				    uint64_t *data);
extern int qca8k_get_sset_count(struct dsa_switch *ds, int port, int sset);

/* Common eee function */
extern int qca8k_set_mac_eee(struct dsa_switch *ds, int port, struct ethtool_eee *eee);
extern int qca8k_get_mac_eee(struct dsa_switch *ds, int port, struct ethtool_eee *e);

/* Common bridge function */
extern void qca8k_port_stp_state_set(struct dsa_switch *ds, int port, u8 state);
extern int qca8k_port_pre_bridge_flags(struct dsa_switch *ds, int port,
				       struct switchdev_brport_flags flags,
				       struct netlink_ext_ack *extack);
extern int qca8k_port_bridge_flags(struct dsa_switch *ds, int port,
			    struct switchdev_brport_flags flags,
			    struct netlink_ext_ack *extack);
extern int qca8k_port_bridge_join(struct dsa_switch *ds, int port,
			   struct dsa_bridge bridge,
			   bool *tx_fwd_offload,
			   struct netlink_ext_ack *extack);
extern void qca8k_port_bridge_leave(struct dsa_switch *ds, int port,
				    struct dsa_bridge bridge);

/* Common port enable/disable function */
extern int qca8k_port_enable(struct dsa_switch *ds, int port,
			     struct phy_device *phy);
extern void qca8k_port_disable(struct dsa_switch *ds, int port);

/* Common MTU function */
extern int qca8k_port_change_mtu(struct dsa_switch *ds, int port, int new_mtu);
extern int qca8k_port_max_mtu(struct dsa_switch *ds, int port);

/* Common fast age function */
extern void qca8k_port_fast_age(struct dsa_switch *ds, int port);
extern int qca8k_set_ageing_time(struct dsa_switch *ds, unsigned int msecs);

/* Common FDB function */
extern int qca8k_port_fdb_insert(struct qca8k_priv *priv, const u8 *addr,
				 u16 port_mask, u16 vid);
extern int qca8k_port_fdb_add(struct dsa_switch *ds, int port,
			      const unsigned char *addr, u16 vid,
			      struct dsa_db db);
extern int qca8k_port_fdb_del(struct dsa_switch *ds, int port,
			      const unsigned char *addr, u16 vid,
			      struct dsa_db db);
extern int qca8k_port_fdb_dump(struct dsa_switch *ds, int port,
			       dsa_fdb_dump_cb_t *cb, void *data);
extern int qca8k_fdb_del(struct qca8k_priv *priv, const u8 *mac,
		  u16 port_mask, u16 vid);
extern int qca8k_fdb_next(struct qca8k_priv *priv, struct qca8k_fdb *fdb,
			  int port);
extern int qca8k_fdb_access(struct qca8k_priv *priv, enum qca8k_fdb_cmd cmd,
			    int port);
extern int qca8k_fdb_search_and_insert(struct qca8k_priv *priv, u8 port_mask,
				       const u8 *mac, u16 vid, u8 aging);
extern int qca8k_fdb_search_and_del(struct qca8k_priv *priv, u8 port_mask,
				    const u8 *mac, u16 vid);

/* Common MDB function */
extern int qca8k_port_mdb_add(struct dsa_switch *ds, int port,
			      const struct switchdev_obj_port_mdb *mdb,
			      struct dsa_db db);
extern int qca8k_port_mdb_del(struct dsa_switch *ds, int port,
		       const struct switchdev_obj_port_mdb *mdb,
		       struct dsa_db db);

/* Common port mirror function */
extern int qca8k_port_mirror_add(struct dsa_switch *ds, int port,
				 struct dsa_mall_mirror_tc_entry *mirror,
				 bool ingress, struct netlink_ext_ack *extack);
extern void qca8k_port_mirror_del(struct dsa_switch *ds, int port,
				  struct dsa_mall_mirror_tc_entry *mirror);

/* Common port VLAN function */
extern int qca8k_port_vlan_filtering(struct dsa_switch *ds, int port,
				     bool vlan_filtering,
				     struct netlink_ext_ack *extack);
extern int qca8k_vlan_add(struct qca8k_priv *priv, u8 port, u16 vid,
			  bool untagged);
extern int qca8k_vlan_del(struct qca8k_priv *priv, u8 port, u16 vid);
extern int qca8k_port_vlan_add(struct dsa_switch *ds, int port,
			       const struct switchdev_obj_port_vlan *vlan,
			       struct netlink_ext_ack *extack);
extern int qca8k_port_vlan_del(struct dsa_switch *ds, int port,
			       const struct switchdev_obj_port_vlan *vlan);

/* Common port LAG function */
extern int qca8k_port_lag_join(struct dsa_switch *ds, int port, struct dsa_lag lag,
			       struct netdev_lag_upper_info *info,
			       struct netlink_ext_ack *extack);
extern int qca8k_port_lag_leave(struct dsa_switch *ds, int port,
			 	struct dsa_lag lag);
