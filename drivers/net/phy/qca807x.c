...
#define PSGMIIPHY_MII_ADDR                              0x05
...
static struct phy_device *psgmii_phy = NULL;
...
static LIST_HEAD(phys_to_calibrate);

#define for_each_phy_to_calibrate(phy_port) list_for_each_entry(phy_port, &phys_to_calibrate, list)
...
static inline
struct phy_device *get_psgmii_phy(void)
{
	return psgmii_phy;
}
...
static int qca807x_config(struct phy_device *phydev)
{
	...
	/* Add to list of PHYs to calibrate */
	phy_calib = kzalloc(sizeof(*phy_calib), GFP_KERNEL);
	if (!phy_calib) {
		return -ENOMEM;
	}

	phy_calib->phydev = phydev;
	list_add(&phy_calib->list, &phys_to_calibrate);
...
static int qca807x_probe(struct phy_device *phydev)
{
	...
	if (!psgmii_phy) {
		psgmii_phy = get_phy_device(phydev->mdio.bus, PSGMIIPHY_MII_ADDR, 0);
		if (!psgmii_phy) {
			dev_err(&phydev->mdio.dev, "Could not fetch PSGMII PHY device");
			return -EINVAL;
		}
	}
...
static int
qca807x_wait_for_phy_link_state(struct phy_device *phy, int need_link)
{
        u16 status;
        int ret;
...
static int qca807x_set_loopback(struct phy_device *phydev, bool enable)
{
	if (enable) {
		phy_write(phydev, MII_BMCR, BMCR_ANENABLE | BMCR_RESET);
		phy_modify(phydev, MII_BMCR, BMCR_PDOWN, BMCR_PDOWN);
		qca807x_wait_for_phy_link_state(phydev, 0);
...
static int qca807x_calibration_start(struct phy_device *phydev)
{
	struct phy_device *psgmii_phy = get_psgmii_phy();
	int val, ret;

	dev_dbg(&phydev->mdio.dev, "starting PSGMII calibration...\n");
        /* Fix PSGMII RX 20bit */
        ret = phy_clear_bits(psgmii_phy, PSGMII_RSTCTRL,
                             PSGMII_RSTCTRL_RX20);
        /* Reset PHY PSGMII */
        ret = phy_clear_bits(psgmii_phy, PSGMII_RSTCTRL,
                             PSGMII_RSTCTRL_RST);
        /* Release PHY PSGMII reset */
        ret = phy_set_bits(psgmii_phy, PSGMII_RSTCTRL,
                           PSGMII_RSTCTRL_RST);

        /* Poll for VCO PLL calibration finish - Malibu(QCA8075) */
        ret = phy_read_mmd_poll_timeout(psgmii_phy,
                                        MDIO_MMD_PMAPMD,
	...
	ret = phy_clear_bits(psgmii_phy, PSGMII_CDRCTRL,
			     PSGMII_CDRCTRL_RELEASE);

        return ret;
}

static void qca807x_port_prep_test(struct phy_device *phydev)
{
	/* put PHY port in loopback */
        qca807x_set_loopback(phydev, 1);
        qca807x_wait_for_phy_link_state(phydev, 1);

        /* enable broadcasts on MDIO bus */
        phy_set_bits_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_MDIO_BRDCST_WRITE,
                         QCA8075_MMD7_MDIO_BRDCST_WRITE_EN);

        /* enable PHY CRC checker and packet counters */
        phy_write_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_CRC_AND_PKTS_COUNT,
                      QCA8075_MMD7_CNT_FRAME_CHK_EN | QCA8075_MMD7_CNT_SELFCLR);
        qca807x_wait_for_phy_link_state(phydev, 1);

        /* set number of packets to send during the test */
        phy_write_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_PKT_GEN_PKT_NUMB,
                      QCA8075_PKT_GEN_PKTS_COUNT);
        /* set packet size */
        phy_write_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_PKT_GEN_PKT_SIZE,
                      QCA8075_PKT_GEN_PKTS_SIZE);
}

static void qca807x_port_unprep_test(struct phy_device *phydev)
{

        /* set packet count to 0 */
        phy_write_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_PKT_GEN_PKT_NUMB, 0);

        /* disable CRC checker and packet counter */
        phy_write_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_CRC_AND_PKTS_COUNT, 0);

        /* disable traffic gen */
        phy_write_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_PKT_GEN_CTRL, 0);

        /* disable broadcasts on MDIO bus */
        phy_clear_bits_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_MDIO_BRDCST_WRITE,
                           QCA8075_MMD7_MDIO_BRDCST_WRITE_EN);

        qca807x_set_loopback(phydev, 0);
}

static int
qca807x_get_calibration_test_results(struct phy_device *phydev)
{
        ...
        /* check counters */
        tx_ok = phy_read_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_EG_FRAME_RECV_CNT_LO);
        tx_ok_high16 = phy_read_mmd(phydev, MDIO_MMD_AN,
                                    QCA8075_MMD7_EG_FRAME_RECV_CNT_HI);
        tx_errors = phy_read_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_EG_FRAME_ERR_CNT);
        rx_ok = phy_read_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_IG_FRAME_RECV_CNT_LO);
        rx_ok_high16 = phy_read_mmd(phydev, MDIO_MMD_AN,
                                    QCA8075_MMD7_IG_FRAME_RECV_CNT_HI);
        rx_errors = phy_read_mmd(phydev, MDIO_MMD_AN, QCA8075_MMD7_IG_FRAME_ERR_CNT);
        tx_all_ok = tx_ok + (tx_ok_high16 << 16);
        rx_all_ok = rx_ok + (rx_ok_high16 << 16);

        tx_loss = QCA8075_PKT_GEN_PKTS_COUNT - tx_all_ok;
        rx_loss = QCA8075_PKT_GEN_PKTS_COUNT - rx_all_ok;
        if (tx_loss | rx_loss | tx_errors | rx_errors) {
		dev_dbg(&phydev->mdio.dev,
			"PSGMII calibration: failed post-calibration test, errors: %d %d %d %d\n",
			tx_loss, rx_loss, tx_errors, rx_errors);
		return -EFAULT;
        }
        return 0;
}
...
static void
qca807x_wait_for_phy_pkt_gen_fin(struct phy_device *phy)
{
        int val;
...
static int
qca807x_start_parallel_pkt_gen(struct mii_bus *bus)
{
        struct phy_device *phy;

        phy = phy_device_create(bus, QCA8075_MDIO_BRDCST_PHY_ADDR,
                                0, 0, NULL);
        if (!phy) {
                pr_err("unable to create mdio broadcast PHY(0x%x)\n",
...
static int qca807x_parallel_test(struct phy_device *phydev)
{
	struct phy_calib *phy_port;
	int result;

	for_each_phy_to_calibrate(phy_port) {
		qca807x_port_prep_test(phy_port->phydev);
	}

	qca807x_start_parallel_pkt_gen(phydev->mdio.bus);

	for_each_phy_to_calibrate(phy_port) {
		/* wait for test results */
		qca807x_wait_for_phy_pkt_gen_fin(phy_port->phydev);
		result = qca807x_get_calibration_test_results(phy_port->phydev);
		if (result) {
			goto end;
		}
	}
	...
	for_each_phy_to_calibrate(phy_port) {
		qca807x_port_unprep_test(phy_port->phydev);
	}
	return result;
}

static int qca807x_serial_test(struct phy_device *phydev)
{
	struct phy_calib *phy_port;
	int result;
...
static int qca807x_calibration_stop(struct phy_device *phydev)
{
	struct phy_device *psgmii_phy = get_psgmii_phy();
	int ret;

	/* Release PSGMII RX CDR */
	ret = phy_set_bits(psgmii_phy, PSGMII_CDRCTRL,
			   PSGMII_CDRCTRL_RELEASE);
	...
	/* Run post-calibration tests */
	ret = qca807x_serial_test(phydev);
	if (ret) {
		return ret;
	}

	ret = qca807x_parallel_test(phydev);
...
static struct phy_driver qca807x_drivers[] = {
	...
	},
	{
		...
		.cable_test_start	= qca807x_cable_test_start,
		.cable_test_get_status	= qca807x_cable_test_get_status,
		.calibration_start      = qca807x_calibration_start,
		.calibration_stop       = qca807x_calibration_stop,
	},
	{
